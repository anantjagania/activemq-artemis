...
by setting the environment variable `ARTEMIS_EXTRA_LIBS` with the directory that contain the jar file.
...
directories.
The environment variable `ARTEMIS_EXTRA_LIBS` is a comma separated list of directories that contains jar files, i.e.
```
