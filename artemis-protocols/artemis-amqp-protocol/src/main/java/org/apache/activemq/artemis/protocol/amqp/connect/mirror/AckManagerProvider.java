...
public class AckManagerProvider {
   ...
   public static AckManager getManager(ActiveMQServer server, int retryTimeout) throws Exception {
      synchronized (managerHashMap) {
         AckManager ackManager = managerHashMap.get(server);
         if (ackManager != null) {
            logger.debug("AckManager was previously defined with retryTimeout={}, requesting with {}", ackManager.getPeriod(), retryTimeout);
            if (retryTimeout > 0 && (retryTimeout < ackManager.getPeriod() || ackManager.getPeriod() < 0)) {
               ackManager.setPeriod(retryTimeout);
            }
            if (ackManager.isStarted()) {
               ackManager.restart();
            }
            return ackManager;
         }

         ackManager = new AckManager(server, retryTimeout);
