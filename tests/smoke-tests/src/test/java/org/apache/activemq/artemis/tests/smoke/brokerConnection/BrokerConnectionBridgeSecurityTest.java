...
public class BrokerConnectionBridgeSecurityTest extends SmokeTestBase {
   ...
   public static void createServers() throws Exception {
      ...
      if (!server0Location.exists()) {
         ...
         cliCreateServer.setAllowAnonymous(false).setUser("A").setPassword("A").setNoWeb(true).setConfiguration("./target/classes/servers/brokerConnect/bridgeSecurityA").setArtemisInstance(server0Location);
         cliCreateServer.createServer();
      }

      if (!server1Location.exists()) {
         CLICreate cliCreateServer = new CLICreate();
         cliCreateServer.setAllowAnonymous(false).setUser("A").setPassword("A").setNoWeb(true).setConfiguration("./target/classes/servers/brokerConnect/bridgeSecurityB").setArtemisInstance(server1Location);
         cliCreateServer.createServer();
      }
   }
