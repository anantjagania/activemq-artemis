...
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQBytesCompatibleMessage;
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQCompatibleMessage;
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQMapCompatibleMessage;
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQObjectCompatibleMessage;
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQStreamCompatibleMessage;
import org.apache.activemq.artemis.jms.contract.compatible1X.ActiveMQTextCompatibleMessage;
...
public class ActiveMQSession implements QueueSession, TopicSession {
      ...
      try {
         ...
         if (dest.isQueue()) {
            ...
            if (!response.isExists() || !response.getQueueNames().contains(getCoreQueueName(dest))) {
               if (response.isAutoCreateQueues()) {
                  try {
                     ...
                     // The queue was created by another contract/admin between the query check and send create queue packet
                  }
               } else {
            ...
            if (subscriptionName == null) {
               ...
               if (connection.getClientID() == null) {
                  throw new IllegalStateException("Cannot create durable subscription - contract ID has not been set");
               }
               ...
               if (!subResponse.isExists()) {
                  ...
                  // A contract can change an existing durable subscription by
    ...
    *
    * @param queueConfiguration the provided queue configuration the contract wants to set
