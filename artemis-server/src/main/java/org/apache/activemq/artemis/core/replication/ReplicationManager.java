...
public final class ReplicationManager implements ActiveMQComponent {
   ...
   public synchronized void start() throws ActiveMQException {
      ...
      if (scheduledExecutorService != null) {
         int period = (int) (timeout / 10);
         if (period > 1000) {
            period = 1000;
         } else if (period < 100) {
            logger.warnf("The cluster call timeout is too low ie %d ms: consider raising it to save CPU", timeout);
            period = 100;
         }
         logger.debugf("Slow replication checker is running with a period of %d ms", period);
         ...
         slowReplicationChecker = scheduledExecutorService.scheduleAtFixedRate(this::checkSlowReplication, period, period, TimeUnit.MILLISECONDS);
      }
   ...
   private void checkSlowReplication() {
      ...
      final long elapsedTimeNotWritable = System.currentTimeMillis() - notWritableFrom;
      if (elapsedTimeNotWritable >= timeout) {
         notWritableFrom = -1;
         releaseReplicatedPackets(replicatePacketRequests);
         try {
